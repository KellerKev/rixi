...
| **"Serve my fine-tuned model behind an OpenAI-compatible API."** | Deploy the model as a keep-alive task, put the proxy in front, and call it from any OpenAI client. |
...
| **OpenAI-compatible inference front door** | serve a model as a task, call it from any OpenAI client |
...
source, in this repo. Your laptop and every compute box dial **out** to one rendezvous host, so
the gateway is the only machine that needs a public IP:

- **On-demand provisioning + auto-teardown** — declare boxes by name in TOML; the gateway spins
  them up via OpenTofu and reclaims them on idle / TTL / release. Automate the whole "get a box, use
  it, stop paying" loop.
- **Brokered end-to-end encryption** — it routes client↔server traffic but sees only ciphertext it
  can't read.
- **Policy, RBAC & audit** — an admin policy floor users can only *tighten*, JWT roles, and a
  queryable DuckDB/OTLP audit trail, with an admin API + web console.
