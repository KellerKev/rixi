...
class JWTAuthenticator:
    def __init__(self, config: Dict[str, Any]):
        ...
        # Pin the algorithm to a safe symmetric allowlist. The proxy authenticates with a
        # shared secret_key, so only HMAC algorithms make sense; never accept "none" or an
        # asymmetric alg, and never derive the algorithm from the token header.
        allowed = {"HS256", "HS384", "HS512"}
        configured = self.jwt_config.get("algorithm", "HS256")
        if configured not in allowed:
            raise ValueError(f"proxy jwt.algorithm must be one of {sorted(allowed)}, got {configured!r}")
        self.algorithm = configured
