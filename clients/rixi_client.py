...
def _resolve_server_url(cli_server: Optional[str], cfg: Dict[str, Any], cli_bearer: Optional[str],
    ...
    if allow_token_url:
        ...
        if token_result:
            ...
            print("=" * 70)
            print("⚠️ SERVER URL DERIVED FROM BEARER TOKEN (--server-from-token)")
...
def _validate_offline_prerequisites(path: str, auto_approve: bool = False) -> bool:
    ...
    print("✅ Offline prerequisites validated")
...
def _show_package_stats(package_stats: dict, offline_mode: bool, package_path: str):
    ...
    if offline_mode:
        print("  🔒 Mode: Offline (dependencies included)")
    else:
        print("  🌐 Mode: Online (dependencies will be downloaded)")
...
class MCPBackChannelHandler:
    ...
    def configure_external_servers(self, config: Dict[str, Any]):
        ...
        if not external_config.get("enabled", False):
            print("🔒 External MCP servers disabled - using built-in handlers only")
            return
            
        if not AIOHTTP_AVAILABLE:
            print("❌ External MCP servers require aiohttp - install with: pip install aiohttp")
            return
            
        self.external_proxy = MCPExternalProxy(config)
        print("🌐 External MCP server routing enabled")
            ...
            try:
                ...
                if result.get("success", True):
                    ...
                    if external_config.get("fallback_to_builtin", True):
                        print("⤵️ External server failed, falling back to built-in handler")
                    else:
...
class OneStepPixiClient:
    ...
    def deploy_task_with_full_features(self, task_name: str, full_config: Dict[str, Any], offline_mode: bool = False) -> str:
        ...
        if proxy_config.get("enabled"):
            ...
            if enable_test_server:
                ...
                print("⏭️  Local test server disabled (config: enable_local_test_server = false)")
    ...
    def _create_mcp_config(self, full_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if encryption_enabled:
            print("🔐 MCP + Proxy back-channel will use AES-256 encryption")
        else:
            print("⚠️ MCP + Proxy back-channel will be UNENCRYPTED (no AES key available)")
    ...
    def _display_proxy_test_info(self):
        ...
        print("📡 Tunnel: Remote Task ↔ Encrypted Channel ↔ Local Services")
        print()
        ...
        if mapping_info["mappings"]:
            ...
            if remote_path.endswith("/*"):
                ...
                print("        -d '{\"message\":\"Hello from remote task!\"}' \\")
...
def main():
    ...
    # Leave the token unset by default; resolve via precedence (CLI > env > config)
    ...
    _auth_headers = _hdrs(bearer_token)
    ...
    if args.show_mcp:
        ...
        if filesystem_config.get("enabled", False):
            ...
            print("Built-in Filesystem Server: ✅ Enabled")
            print(f"  Root Path: {root_path}")
        else:
            print("Built-in Filesystem Server: ❌ Disabled")
        ...
        if external_config.get("enabled", False):
            ...
            print("External MCP Servers: ❌ Disabled")
...
actions = ["sql_*", "schema_*"]
    ...
    if args.show_proxy:
        ...
        if enable_test_server_config is not None:
            ...
            print("Local Test Server (config): default (enabled)")
        ...
        print("\n💡 Quick Test:")
        print("   1. Start local server: python local_test_server.py")  
        print("   2. Deploy task: python enhanced_client.py --task your_task --with-proxy")
        print(f"   3. Test from remote: curl http://localhost:{remote_port}/api/health")
        
        print("\n🔧 Test Server Control:")
        print("   Config file:  enable_local_test_server = true/false")
        print("   CLI enable:   --local-test-server (overrides config)")
        print("   CLI disable:  --no-local-test-server (overrides config)")
    ...
    if args.aes_key and Path(args.aes_key).exists():
        try:
            ...
            if args.handshake_secret:
                print("💡 Skipping handshake - using existing key file instead")
    ...
    if args.attach_history:
        ...
        if features_enabled.get("mcp"):
            ...
            if filesystem_enabled:
                ...
                print("    Built-in Filesystem: ❌ Disabled")
            ...
            if external_enabled:
                ...
                if servers:
                    ...
                    print("    ⚠️ External servers enabled but no servers configured")
            else:
                print("    External Servers: ❌ Disabled")
        ...
        if features_enabled.get("proxy"):
            ...
            if mappings:
                ...
                print("🔒 Headers will be transformed and local tokens injected automatically")
                
                if test_server_enabled:
                    print("🧪 Local test server will be available for immediate testing")
                else:
                    print("⏭️  Local test server disabled - testing only available from remote tasks")
                print()
        ...
        if offline_mode:
            print("    Dependencies: Will be packaged locally")
            print("    Server Requirements: No internet connection needed")
            if validate_deps:
                print("    Validation: Pre-flight checks enabled")
        else:
            print("    Dependencies: Will be downloaded on server")
            print("    Server Requirements: Internet connection required")
        ...
        if features_enabled or offline_mode:
            ...
            try:
                ...
                if mode == "keep_alive":
                    ...
                    try:
                        time.sleep(timeout)
                        print("⏰ Timeout reached, exiting gracefully")
                    except KeyboardInterrupt:
                        print("\n🛑 Interrupted by user")
