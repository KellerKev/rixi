...
_HKDF_INFO_JWT = b"rixi-smcp-v3-jwt"
...
def _derive_jwt_secret(secret: str, kdf_salt: str = "") -> str:
    """Deterministic value derived from the shared channel secret.

    SECURITY: do NOT use this to sign session JWTs. Every client must hold the
    channel ``secret_key`` to use the encrypted transport, so a signing key
    derived from it can be recomputed by any client to forge a token and bypass
    the api_key gate (this server's only auth). The server signs with an
    independent secret (explicit ``jwt_secret`` or a random per-process key).
    Retained only so tests can assert such a forged token is rejected."""
    from cryptography.hazmat.primitives import hashes
    from cryptography.hazmat.primitives.kdf.hkdf import HKDF

    salt = kdf_salt.encode() if kdf_salt else _DEFAULT_KDF_SALT
    master = hashlib.pbkdf2_hmac("sha256", secret.encode(), salt, _PBKDF2_ITERS, dklen=32)
    jwt_key = HKDF(algorithm=hashes.SHA256(), length=32, salt=None, info=_HKDF_INFO_JWT).derive(master)
    return base64.urlsafe_b64encode(jwt_key).decode()
...
class SMCPToolServer:
    ...
    def __init__(self, tools: Dict[str, Tool], *, secret_key: str,
                 kdf_salt: str = "",
                 jwt_secret: str = "", node_id: str = "rixi",
        ...
        # client holds secret_key to use the channel, so deriving the signing
        # key from it would let any client forge a session token and bypass the
        # api_key gate. Use an explicit jwt_secret when set (stable across
        # restarts/replicas); otherwise a random per-process key.
        if not jwt_secret:
            jwt_secret = secrets.token_urlsafe(32)
        # Refuse to run unauthenticated unless the operator explicitly opts in.
        if not api_key and not api_keys and not allow_open:
            raise RuntimeError(
                "smcp server has no api_key/api_keys configured; pass allow_open=True "
                "(CLI: --allow-open) to intentionally accept any client (dev only)")
        self.tools = tools
    ...
    def _resolve_agent(self, provided: str) -> Optional[str]:
        # Open dev mode only when explicitly enabled and no keys are configured.
        if self.allow_open and not self.api_keys and not self.api_key:
