#!/usr/bin/env python3
"""
AI Agent Framework - Connect to remote inference APIs via Pixi Runner
"""
        ...
        for chunk in response.iter_content(chunk_size=4096):
            ...
            while True:
                if need is None and len(buf) >= 4:
                    ...
                    buf = buf[4:]
                if len(buf) < need or (need is not None and len(buf) < need):
                    break

                enc, buf = buf[:need], buf[need:]
                try:
                    print(f"Processing encrypted chunk of length: {len(enc)}")
...
class HaikuAgent(Agent):
    ...
    def generate_haiku(self, topic, output_file="haiku.txt"):
        """Generate a haiku about the given topic and save it locally."""
        ...
        # Send request with the request_id
        print(f"Sending request with ID: {self.request_id}")
        self.send_to_remote({
            "command": "generate",
            "prompt": self.current_prompt,
            "request_id": self.request_id  # Include request ID to match the response
        })
        
        # Set up callback that only matches the response with our request_id
            ...
            try:
                if "output" in data:
                    ...
                    try:
                        ...
                        # This might be a partial or invalid JSON, just skip it
                        pass
            except Exception as e:
        ...
        # Receive the response, but with a timeout
        import time
        start_time = time.time()
        max_wait_time = 30  # seconds
        
        # Try to get the response from the stream
        self.receive_from_remote(response_callback)
        
        # If no response received yet, try a direct query approach
        if not self.response_data and (time.time() - start_time < max_wait_time):
            ...
            # Loop until we get our response or timeout
            while not self.response_data and (time.time() - start_time < max_wait_time):
                ...
                # Check recent output for our request_id
                ...
                # If found, break out, otherwise wait a bit and try again
                if self.response_data:
                    break
                time.sleep(1)
        
        # Perform local action to save the haiku
        if self.response_data:
            self.perform_local_action("write_file",
    ...
    def perform_local_action(self, action_type, **params):
        """Perform local actions like writing files."""
...
# Read pixi_remote_config.toml file for auth credentials (same as client.py)
def read_pixi_config():
    """Read the pixi_remote_config.toml file for configuration."""
...
def create_auth_headers(bearer=None, snowflake=None):
    """Create authentication headers from tokens."""
    headers = {}
    if snowflake:
...
def main():
    ...
    # Read config file for auth tokens
    config = read_pixi_config()

    # Setup authentication - prioritize command line args over config file
    ...
    # Print a message about authentication
    if auth_headers:
        print("Using authentication from config")
