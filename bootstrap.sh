...
# The rixi server spawns tasks with a bare `pixi run …`, so pixi's bin dir must be on PATH for
# the server process (and its task subprocesses).
...
Environment=PATH=$PIXI_BIN_DIR:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
