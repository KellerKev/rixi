...
async def extract_and_run(pkg: str, task: str, tid: Optional[str]) -> AsyncGenerator[bytes, None]:
    ...
    if is_offline:
        ...
        print("🚀 Starting task with offline dependencies (no internet required)")
        yield send_frame((json.dumps({
        ...
        print("🚀 Starting task - pixi will download dependencies")
        yield send_frame((json.dumps({
...
if __name__ == "__main__":
    ...
    p.add_argument("--tls-cert",
                    help="Path to a TLS certificate (PEM). Serve HTTPS directly instead of "
                         "relying on a reverse proxy. Requires --tls-key.")
    ...
    if args.host not in loopback_hosts and not tls_enabled and aes_key is None:
        ...
        print("⚠️  Transport is plaintext HTTP. Use --tls-cert/--tls-key, an AES key "
              "(--aes-key/--key-secret), or terminate TLS at a reverse proxy.")
