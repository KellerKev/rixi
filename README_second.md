...
**RIXI** — *Remote Interaction and Execution Implementation* — is a **secure remote-compute plane for [Pixi](https://pixi.sh/) projects**. A client packages a project *and its resolved environment*, ships it to a server over an encrypted, authenticated channel, and the server runs it as an isolated task and streams the output back. Think **a secure, self-hostable alternative to Modal / SkyPilot / Runhouse** for running training, inference, and data jobs on your own boxes — where Metaflow leans on AWS Batch/Kubernetes, RIXI sits in that spot.

Optional components add on-demand cloud provisioning (a **gateway** control plane), a multi-agent engine, an OpenAI-compatible inference proxy, and a pluggable model backend, with [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) for extensible tool access.

> **Free and open source (MIT).** Everything here — execution engine, clients, agent, proxy,
> inference backend, tunnel, and the **gateway control plane** (provisioning, RBAC/policy, audit,
> admin console) — is yours to self-host and build on.

## Install

```bash
pip install rixi          # client SDK + `rixi` CLI (notebook-friendly)
```

```python
from rixi import Client
rixi = Client("http://127.0.0.1:9000", token="…")   # token optional on loopback
print(rixi.run(".", task="train").output)            # package → ship → run → stream back
```

Server-side components (server, gateway, tunnel, agent, proxy, inference) run with Pixi from their
directories — see [Quickstart](#quickstart-local). Prefer containers? See [Docker](#docker).
...
Beyond the single-server core, the [`gateway/`](gateway/) turns RIXI into **managed
infrastructure** — many users, many disposable cloud boxes, no inbound ports, centrally governed.
It never forks the execution engine; it drives the core over its public API and reuses the open
tunnel wire format. All open source, in this repo.

| Capability | What it does |
|------------|--------------|
| **Rendezvous / broker** | A single authenticated host. Your laptop and every compute box dial *out* to it — so the gateway is the **only** host that needs a public IP and one open port; nothing else opens an inbound port. |
| **On-demand cloud provisioning** | Declare boxes by name in a simple TOML catalog; the gateway spins them up on demand (via OpenTofu), with warm-reuse or fresh-per-request, and auto-teardown (`idle` / `ttl` / `on_release`). |
| **Brokered end-to-end encryption** | The gateway brokers client↔server connections but, with RIXI's key handshake, sees only AES-256-GCM ciphertext — true end-to-end encryption through a broker it needn't be trusted with. |
| **Policy, RBAC & quotas** | An admin-authored policy floor users can only *tighten*: JWT roles, per-resource requirements (`require_e2e` / `require_jwt` / `require_tls`), provider/region allowlists, and concurrency/total quotas. |
| **Audit & OpenTelemetry** | Every security-relevant action recorded as a structured audit event, fanned out to OTLP, a queryable DuckDB history, and a JSON log. |
...
See [`gateway/README.md`](gateway/README.md) to run it.

## How RIXI compares

| | RIXI | Modal | SkyPilot | Runhouse | Metaflow |
|---|---|---|---|---|---|
| Self-hostable, no vendor account | ✅ | ❌ (hosted) | ✅ | ✅ | ✅ |
| Ships resolved env (Pixi/conda) | ✅ | partial | via images | ❌ | ❌ |
| Reach firewalled boxes (outbound-only) | ✅ tunnel/gateway | n/a | ❌ | ❌ | ❌ |
| Air-gapped / offline bundle | ✅ | ❌ | ❌ | ❌ | ❌ |
| On-demand cloud provisioning | ✅ gateway | ✅ | ✅ | ✅ | via Batch/K8s |
| OpenAI-compatible inference front door | ✅ proxy | ❌ | ❌ | ❌ | ❌ |
| Governance (RBAC/policy/audit) | ✅ gateway | ✅ (SaaS) | partial | ❌ | partial |

RIXI's niche: a secure remote **executor** you fully control — strongest when boxes are firewalled,
environments must be reproduced exactly, or the network is air-gapped. It is not a hosted SaaS and
not (yet) a DAG orchestrator; pair it with your scheduler of choice.

## Docker

Run the server in a container instead of installing Pixi on the host:

```bash
docker build -t rixi-server .
docker run --rm -p 9000:9000 rixi-server            # loopback-auth defaults; see Dockerfile
```

The image bundles the server component and Pixi. Mount credentials/keys and pass server flags
(`--public-key`, `--aes-key`, `--tls-cert`) as needed — see [`Dockerfile`](Dockerfile).
