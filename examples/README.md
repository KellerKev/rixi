...
| [`agent-demos/`](agent-demos/) | Drives the agent framework in [`../agent/`](../agent/): a platform launcher, a cross-platform comparison, and a printable list of `start_agent.py` usage examples. The fully-populated demo configs (`agent_config.yaml`, `haiku_config.yaml`) live in this folder. | The scripts import the agent modules from `../agent` (they add it to the path) and read the configs alongside them; run with the agent dependencies installed. `bash agent-demos/usage_examples.sh` prints example commands. |
